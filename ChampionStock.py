...
# print(dailyExchange_df)
'''
# 本益比(Price-to-Earning Ratio)
PE = pd.to_numeric(df['本益比'], errors='coerce') < 15
DY = pd.to_numeric(df['殖利率(%)'], errors='coerce') > 5  # 殖利率(Dividend yield)
# 淨價比(Price-to-Book Ratio) 找到股價淨值比小於0.7的股票
PBR = pd.to_numeric(df['股價淨值比'], errors='coerce') <= 1.81
candidate = df[(PE & DY & PBR)]
print(candidate)
'''
...
with db_session:
    ...
    data = []
    for row in rows:
        data.append([entry.replace("'", "").strip() for entry in str(row).replace("(", "").replace(")", "").split(",")])
    print(data)
        

    stockInfo_df = pd.DataFrame(rows, columns=[["證券代號", "證券名稱", "產業別", "年/季", "營收(億)", "稅後淨利(億)", "毛利(%)", "營業利益(%)", "稅後淨利(%)", "ROE(%)", "EPS(%)", "現金股利", "股票股利", "合計股利"]])
    print(stockInfo_df)
    g = stockInfo_df.groupby('證券代號')
