...
def DownloadChip(stockId):
    ...
    response = session.get(base_url + 'bsMenu.aspx', headers=headers)
    if response.status_code == 200:
        ...
        img = UtilsCaptcha.GetCaptcha(base_url + img_url)
        ...
        resp = session.post('https://bsr.twse.com.tw/bshtm/bsMenu.aspx', data=params, headers=headers)
