...
#url = 'https://www.twse.com.tw/exchangeReport/STOCK_DAY?response=csv&date=20211011&stockNo=1409'
# data = pd.read_csv(url, encoding='big5')  # use sep="," for coma separation.
# print(data[0:10])
...
def create_df(date, stock_no):
    ...
    s2 = s.drop(['date'], axis=1)  # 刪除日期欄位 (若 axis=1 為欄。若 axis=0 為代表列)
    #mlist = []
    # for item in s2.index:a
...
for i in range(len(listDji)):
    result = create_df('20211101', listDji[i])
    print(result)
