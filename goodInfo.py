...
def GetStockDividend(soup):
    return GetGridData(soup, '#FINANCE_DIVIDEND', 3, 10)
    # print(table)B


# 抓取個股ROE、毛利等、EPS 等..明細
def GetStockDetail(soup):
    return GetGridData(soup, '#FINANCE_INCOME_M > div:nth-child(2) > div:nth-child(1) > table', 0, 10)


def GetDailyTradePrice(stockId):
    url = "https://www.twse.com.tw/exchangeReport/STOCK_DAY?response=json&date=${date}&stockNo="
    res = requests.get(url + stockId, headers=ua.GetHeader())
    res.encoding = "utf-8"
    # print(res.text)
...
for idx, stockId in enumerate(stocks):
    print(stockId)
    # 休息一下再抓, 避免存取過於頻繁被擋
    time.sleep(np.random.randint({True: 0, False: 30}[idx == 0], 90))
