...
@router.get("/DailyExchangeReport/{filter}")
def GetDailyExchangeReport(filter: Optional[bool] = False):
    ...
    if filter:
        ...
        return stockdf.to_json()


@router.get("/DailyExchange/")
# 取出每日收盤價
...
@router.get("/StockCapital/{filter}")
# 資本額
def GetStockCapital(filter: Optional[bool] = False):
...
@router.get("/BasicStockInfo/{filter}")
def GetBasicStockInfo(filter: Optional[bool] = False):
